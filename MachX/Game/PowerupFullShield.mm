...
@implementation PowerupFullShield
@synthesize spriteBody = _spriteBody;
@synthesize game = _game;
@synthesize body = _body;

+ (id) InitFullShield:(Game*)game Position:(CGPoint)position {
...
- (id) initWithGame:(Game*)game Position:(CGPoint)position {
    if (self = [super init]) {
        ...
        self.spriteBody = [CCSprite spriteWithFile:@"FullShield-hd.png"];
        self.spriteBody.tag = 13;
        //self.spriteBody.opacity = 100;
...
- (void) addBody {
    ...
    [[GB2ShapeCache sharedShapeCache] addFixturesToBody:_body forShapeName:@"FullShield-hd"];
