...
//FUCK YEAH
- (void) ratio:(CGPoint)newRatio forNode:(CCSprite *)Node {
    //NSLog(@"Node.tag: %d", Node.tag);
    
    for (int i = 0; i < scrollOffsets->num; i++) {
        CCParallaxScrollOffset *scrollOffset = scrollOffsets->arr[i];
		//CCNode *child = scrollOffset.child;
		//NSLog(@"Checking %@", point.child);
        
		// Found it, modify its ratio
		if(scrollOffset.child == Node)
		{
            //NSLog(@"%f", scrollOffset.ratio.x);

			scrollOffset.ratio = newRatio;
            //NSLog(@"%f", scrollOffset.ratio.x);
		}
    }
    
    //[CCParallaxScrollOffset initChangeRatio:ratio forNode:Node];
}

- (void) ratioChangeOverTime:(CGPoint)newRatio forNode:(CCSprite *)Node delta:(ccTime)dt {
    //NSLog(@"Node.tag: %d", Node.tag);
