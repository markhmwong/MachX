...
+ (id) InitSpeedBoost:(Game*)game Position:(CGPoint)position {
...
- (id) initWithGame:(Game*)game Position:(CGPoint)position {
    if (self = [super init]) {
        ...
        self.spriteBody = [CCSprite spriteWithFile:@"Boost-hd.png"];
...
- (void) addBody {
    ...
    [[GB2ShapeCache sharedShapeCache] addFixturesToBody:_body forShapeName:@"Boost-hd"];
