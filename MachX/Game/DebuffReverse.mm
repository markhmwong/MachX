...
@implementation DebuffReverse
@synthesize spriteBody = _spriteBody;
@synthesize game = _game;
@synthesize body = _body;

+ (id) InitDebuffReverse:(Game*)game Position:(CGPoint)position {
...
- (id) initWithGame:(Game*)game Position:(CGPoint)position {
    if (self = [super init]) {
        ...
        self.spriteBody = [CCSprite spriteWithFile:@"Reverse-hd.png"];
        self.spriteBody.tag = -10;
...
- (void) addBody {
    ...
    [[GB2ShapeCache sharedShapeCache] addFixturesToBody:_body forShapeName:@"Reverse-hd"];
